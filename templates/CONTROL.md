...
9. **DONE** — all STABLE + §4 fully checked + fresh UI evidence for every UI feature (a Profile with no UI — e.g. `surface: cli` or `api` — has no UI features, so this clause is already satisfied) + the IDEATE round already filed → set `greenlight: "yes"` and `status: COMPLETE`, write `greenlight/state/STOP`, and output only: "GREENLIGHT ACHIEVED — the app is finished. Stop the loop and review your app."
